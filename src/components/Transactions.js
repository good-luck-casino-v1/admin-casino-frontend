  ...
  const handleTransactionStatusUpdate = async (transaction, status) => {
    try {
      ...
      // 🟡 CASE 1: Completed — trigger payout
      if (status === "completed") {
        const method = (transaction.payment_method || "").toLowerCase();
        ...
        // ✅ Detect correct payout route
        let payoutEndpoint = "";
        if (method.includes("toppay")) {
          payoutEndpoint = `${API}/api/transactions/admin-payout-toppay`; // TopPay backend
        } else if (method.includes("cloudpay")) {
          payoutEndpoint = `${API}/api/transactions/admin-payout`; // CloudPay backend
        } else {
          setAlert({
            show: true,
        ...
        if (response.data.success) {
          setAlert({
            show: true,
            message: response.data.message || "Payout triggered successfully.",
            variant: "success",
          });
        } else {
      ...
      // 🔴 CASE 2: Reject — normal PUT status update
