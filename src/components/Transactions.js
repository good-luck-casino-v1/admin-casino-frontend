  ...
  try {
    ...
    if (status === "completed") {
      ...
      const payoutData = {
        ...
        payment_method: transaction.payment_method, // ✅ tells backend which gateway
      };

      console.log("🚀 Sending payout to API:", payoutData);
    ...
    setAlert({
      show: true,
      message: error.response?.data?.message || error.message || "Error updating transaction.",
      variant: "danger",
    });
  } finally {
    setProcessing(false);
