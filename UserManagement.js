...
const UserManagement = () => {
  ...
  // New state for edited transaction amounts
  ...
  const handleTransactionStatusUpdate = async (transactionId, status) => {
    try {
      // Check if this is a payment_gateway transaction
      const transaction = viewData[currentUser.id].pendingTransactions.find(tx => tx.id === transactionId);
      
      if (transaction && transaction.payment_method === 'payment_gateway' && status === 'completed') {
        // Get the edited amount or use the original amount if not edited
        const amount = editedTransactionAmounts[transactionId] || transaction.amount;
        
        // Prepare data for UPI withdraw API
        const withdrawData = {
          Amount: amount,
          acc_no: transaction.account_number || viewData[currentUser.id].details.account_number,
          account_name: transaction.account_name || viewData[currentUser.id].details.account_name,
          bank_code: transaction.bank_code || viewData[currentUser.id].details.bank_code,
          ifsc_code: transaction.ifsc_code || viewData[currentUser.id].details.ifsc_code,
  ...
  // Handle edited amount change for payment_gateway transactions
  const handleEditedAmountChange = (transactionId, amount) => {
    setEditedTransactionAmounts(prev => ({
      ...prev,
                      ...
                      <tbody>
                        {viewData[currentUser.id].pendingTransactions.map((tx) => (
                          <tr key={tx.id}>
                            <td>{tx.type}</td>
                            <td>
                              {tx.payment_method === 'payment_gateway' ? (
                                <Form.Control
                                  type="number"
                                  value={editedTransactionAmounts[tx.id] || tx.amount}
                                  onChange={(e) => handleEditedAmountChange(tx.id, e.target.value)}
                                  className="bg-dark text-light editable-amount border-secondary"
                                  size="sm"
                                />
                              ) : (
                                `₹${tx.amount}`
